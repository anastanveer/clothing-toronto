...
@section('content')
  ...
  <!-- ===== Brands ===== -->
  @if (!empty($brands))
    <section class="py-5 kb-brand-section">
      <div class="container text-center">
        <div class="kb-title">Shop by Brand</div>
        <div class="kb-sub">Trusted labels curated for Canada.</div>
        <div class="row g-3 mt-3">
          @foreach ($brands as $brand)
            <div class="col-12 col-md-4">
              <a class="kb-brand-card" data-brand="{{ $brand['key'] }}" href="{{ route('brands.show', ['brand' => $brand['key']]) }}">
                <div class="kb-brand-card-title">{{ $brand['label'] }}</div>
                <div class="kb-brand-card-meta">{{ $brand['tagline'] ?? 'Curated seasonal essentials.' }}</div>
                <div class="kb-brand-card-footer">
                  <span class="kb-brand-count">{{ $brand['count'] ?? 0 }} Items</span>
                  <div class="kb-brand-card-cta">Shop {{ $brand['label'] }}</div>
                </div>
              </a>
            </div>
          @endforeach
        ...
        <div class="kb-slider-track">
          @forelse ($newMenProducts as $product)
            @php
              ...
              $productId = $resolvedBrandKey . '::' . $product->handle;
              $compareValue = null;
  ...
  <!-- ===== Khanabadosh Spotlight ===== -->
  @if (!empty($khanabadoshProducts) && $khanabadoshProducts->count())
    <section class="pb-5">
      <div class="container text-center">
        <div class="kb-title">Khanabadosh Edit</div>
        <div class="kb-sub">Signature articles from Khanabadosh.</div>

        <div class="row g-4 mt-2">
          @foreach ($khanabadoshProducts as $product)
            @php
              $image = optional($product->images->sortBy('position')->first())->src;
              $altImage = optional($product->images->sortBy('position')->skip(1)->first())->src;
              $priceValue = $product->effectivePrice();
              $price = \App\Support\CurrencyFormatter::format($priceValue);
              $productId = 'khanabadosh::' . $product->handle;
              $compareValue = null;
              if ($product->hasActiveDiscount() && $product->price) {
                  $compareValue = $product->price;
              } elseif ($product->compare_at_price && $product->price && $product->compare_at_price > $product->price) {
            ...
            <div class="col-6 col-md-4 col-lg-3">
              <div class="kb-product-card position-relative">
                ...
                @endif
                <div class="kb-product-media">
                  <a class="text-decoration-none text-dark" href="{{ route('brands.products.show', ['brand' => 'khanabadosh', 'collection' => 'all', 'slug' => $product->handle]) }}">
                    @if ($image)
                  ...
                  <div class="kb-product-actions">
                    ...
                    <button class="kb-action-btn js-zoom" type="button"
                      ...
                      data-image="{{ $image }}"
                      data-description="{{ \Illuminate\Support\Str::limit(strip_tags($product->body_html ?? ''), 220) }}"
                      data-url="{{ route('brands.products.show', ['brand' => 'khanabadosh', 'collection' => 'all', 'slug' => $product->handle]) }}"
                      data-product-id="{{ $productId }}"
                      aria-label="Quick view">
                      <i class="bi bi-zoom-in"></i>
                    </button>
        ...
        <div class="mt-4">
          <a class="btn btn-dark px-4 py-2 rounded-3 text-uppercase fw-semibold" style="letter-spacing:.08em;font-size:.8rem;" href="{{ route('brands.show', ['brand' => 'khanabadosh']) }}">
            Shop Khanabadosh
          </a>
        </div>
      </div>
  ...
  <section class="pb-5">
    <div class="container text-center">
      ...
      <div class="kb-tabs">
        ...
        <button class="kb-tab" type="button" data-tab-target="accessories">Accessories</button>
      </div>
      ...
      <div class="kb-tab-panel d-none" data-tab-panel="accessories">
        <div class="kb-slider" data-slider>
          <button class="kb-slider-btn kb-slider-prev" type="button" aria-label="Previous">
            <i class="bi bi-chevron-left"></i>
          </button>
            ...
            @foreach ($accessoryProducts as $product)
              ...
              <div class="kb-slide">
                <div class="kb-product position-relative kb-product-card">
                  ...
                  <div class="kb-product-media">
                    ...
                    <div class="kb-product-actions">
                      ...
                      <button class="kb-action-btn js-zoom" type="button"
                        ...
                        data-image="{{ $image }}"
                        data-description="{{ \Illuminate\Support\Str::limit(strip_tags($product->body_html ?? ''), 220) }}"
                        data-url="{{ route('products.show', ['collection' => 'accessories', 'slug' => $product->handle]) }}"
                        data-product-id="{{ $productId }}"
                        aria-label="Quick view">
                        <i class="bi bi-zoom-in"></i>
                      </button>
                  ...
                  <a class="kb-card-link" href="{{ route('products.show', ['collection' => 'accessories', 'slug' => $product->handle]) }}" aria-label="View {{ $product->title }}"></a>
                  <div class="code">{{ $product->handle }}</div>
                  <div class="now">
                    {{ $price }}
                    @if ($comparePrice)
        ...
        <div class="col-12 col-md-4">
          <div class="item">
            ...
            <div>
              ...
              <div class="d">Free shipping across Canada</div>
            </div>
          </div>
        </div>
        <div class="col-12 col-md-4">
        ...
        <div class="col-12 col-md-4">
          <div class="item">
            ...
            <div>
              ...
              <div class="d">We support online 24/7</div>
            </div>
          </div>
        </div>
      </div>
